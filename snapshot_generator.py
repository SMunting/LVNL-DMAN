...
class HistoricalCSVDataSource:
    ...
    #     return self.df_history[self.df_history['timesec'] <= t].copy()
    ...
    def load_up_to(self, t: pd.Timestamp) -> pd.DataFrame:
        ...
        df = self.df_history[self.df_history['timesec'] < next_minute]
...
def _df_to_slots(df_final: pd.DataFrame, slots_df: pd.DataFrame) -> List[Slot]:
    ...
    # Group by slot and resolve conflicts with +1 second increments
    order = np.lexsort((fk_arr, slot_idx_arr))
    used_ttots: set = set()
...
def generate_day_snapshots(
    ...
    *,
    ...
    frozen_assignments: Dict[str, pd.Timestamp] = {}  # flight_key -> TTOT timestamp
    ...
    # Track TSAT assignments for missed-TSAT detection in pairwise scheduler
    prev_slot_map: Dict[str, Dict[str, Any]] = {}  # Changed from prev_tsat_map to track complete slot info
    ...
    if not verbose and sys.stderr.isatty():  # keep quiet in non-TTY / verbose mode
        pbar = tqdm(total=total_recompute, desc="Snapshots", unit="min")
    ...
    while current <= day_end_utc:
        ...
        if recompute:
            ...
            if ALLOW_LATE_REVIVAL and late_updates_lookup:
                ...
                if to_unfreeze:
                    for fk in to_unfreeze:
                        ...
                        if sl_prev and sl_prev.ttot is not None and PRESERVE_VACATED_SLOTS and fk in tsat_missed_flights:
                            vac_fk = f"VACATED__{sl_prev.flight_key}__{sl_prev.ttot.strftime('%H:%M:%S')}"
                            vac_slot = Slot(
                                flight_key=vac_fk,
                                rwy=sl_prev.rwy,
                                ttot=sl_prev.ttot,
                            ...
                            frozen_assignments[vac_fk] = sl_prev.ttot
            ...
            if use_full:
                # Full (pairwise) pipeline: build per-minute state and run continuous
                # pairwise sequencing implemented in a separate module.
                df_state = _build_state_dataframe(messages)
                try:
                    from pairwise_scheduler import run_pairwise_scheduler
                except Exception:
                    # Fallback to old scheduler if new module unavailable
                    ...
                    scheduled_dicts, vacated_map, new_slot_map = run_pairwise_scheduler(
                        df_state,
                        frozen_assignments,
                        now_ts=current,
                        capacity_blocks=capacity_blocks_df,
                    ...
                    # Update prev_slot_map for next iteration
                    prev_slot_map = new_slot_map
                    # Convert lightweight dicts to Slot dataclass instances used elsewhere
                    scheduled_slots = []
                    for sd in scheduled_dicts:
                        sl = Slot(
                            flight_key=sd.get('flight_key'),
                            rwy=sd.get('rwy'),
                            ttot=sd.get('ttot'),
                            tsat=sd.get('tsat'),
                            ctot=sd.get('ctot'),
                            status=sd.get('status', PLANNED_STATUS),
                            rw_cur=sd.get('rw_seq'),
                            rw_cur_start=None,
                            rw_cur_end=None,
                            tobt=sd.get('tobt'),
                        )
                        scheduled_slots.append(sl)
                    # Materialize vacated placeholders so their slots cannot be reused
                    if PRESERVE_VACATED_SLOTS and vacated_map:
                        for rwy, vac_entries in vacated_map.items():
                            for vac_entry in vac_entries:
                                # vac_entry is now a tuple: (flight_key, old_ttot, old_slot_dict)
                                try:
                                    if isinstance(vac_entry, tuple) and len(vac_entry) == 3:
                                        orig_fk, old_ttot, old_slot = vac_entry
                                        old_ttot = pd.to_datetime(old_ttot)
                                        # Extract preserved timing data
                                        old_tsat = old_slot.get('tsat')
                                        old_tobt = old_slot.get('tobt')
                                        old_ctot = old_slot.get('ctot')
                                    elif isinstance(vac_entry, tuple) and len(vac_entry) == 2:
                                        # Fallback for old 2-tuple format (backward compatibility)
                                        orig_fk, old_ttot = vac_entry
                                        old_ttot = pd.to_datetime(old_ttot)
                                        old_tsat = None
                                        old_tobt = None
                                        old_ctot = None
                                    else:
                                        # Fallback for very old format (just timestamp)
                                        orig_fk = rwy
                                        old_ttot = pd.to_datetime(vac_entry)
                                        old_tsat = None
                                        old_tobt = None
                                        old_ctot = None
                                except Exception:
                                    continue
                                vac_fk = f"VACATED__{orig_fk}__{old_ttot.strftime('%H:%M:%S')}"
                                if vac_fk not in frozen_meta:
                                    vac_slot = Slot(
                                        flight_key=vac_fk,
                                        rwy=rwy,
                                        ttot=old_ttot,
                                    ...
                                    frozen_meta[vac_fk] = vac_slot
                                    frozen_assignments[vac_fk] = old_ttot
            else:
            ...
            for sl in scheduled_slots:
                ...
                if sl.flight_key in frozen_meta:
                    ...
                    if is_past(sl, current):
                        ...
                        if use_full and sl.ttot is not None:
                            frozen_assignments[sl.flight_key] = sl.ttot
